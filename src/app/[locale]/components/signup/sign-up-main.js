      ...
      <div className="w-full bg-[rgb(255,255,255,0.8)] z-30 flex flex-col items-center justify-center px-10 py-10 rounded-lg shadow-lg mx-auto">
        ...
        <form className="flex flex-col w-full">
          <div className="mb-6">
            ...
            <input type="text" id="username" name="username" className="text-sm border rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 max-sm:text-xs" required placeholder={t('placeholderName')}/>
          </div>
          <div className="mb-6">
            <label htmlFor="email" className="block text-gray-700 mb-1 text-xs max-sm:text-[0.7rem]">{t('email')}</label>
            <input type="text" id="email" name="email" className="text-sm border rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 max-sm:text-xs" required placeholder={t('placeholderEmail')}/>
          </div>
          <div className="mb-6">
            <label htmlFor="password" className="block text-gray-700 mb-1 text-xs max-sm:text-[0.7rem]">{t('password')}</label>
            <input type="password" id="password" name="password" className="text-sm border rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 max-sm:text-xs" required placeholder={t('placeholderPassword')}/>
          </div>
          <div className="mb-6">
            <label htmlFor="confirmpassword" className="block text-gray-700 mb-1 text-xs max-sm:text-[0.7rem]">{t('confirmPassword')}</label>
            <input type="password" id="confirmpassword" name="confirmpassword" className="text-sm border rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 max-sm:text-xs" required placeholder={t('placeholderConfirmPassword')}/>
          </div>
