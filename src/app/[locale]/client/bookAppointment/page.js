            ...
            <form onSubmit={handleSubmit} className="w-full flex flex-col items-center gap-4">
                ...
                <div className="w-full border-2 border-gray-200 rounded-lg py-8 px-5 shadow-md relative mb-5 flex gap-4 justify-between max-md:flex-col max-md:gap-2">
                    <div className="flex-1 relative">
                        ...
                        <input type="date" id="fecha" name="fecha" value={formData.fecha} onChange={handleChange} className="p-2 border-2 border-gray-300 shadow-md rounded-lg"></input>
                    </div>
                    <div className="flex-1 relative">
                ...
                <div className="w-full border-2 border-gray-200 rounded-lg py-8 px-5 shadow-md relative mb-5">
                    ...
                    <div className="flex flex-col gap-4 bg-gray-50 p-4 rounded-lg shadow-sm border-2 border-gray-300 border-dashed"> 
                        ...
                        Servicio: <span className="font-normal">{servicioSeleccionado ? servicioSeleccionado.Nombre : '-'}</span>
                        </p>
                        <p className="text-sm font-bold">
                        Terapeuta: <span className="font-normal">{terapeutaSeleccionado ? terapeutaSeleccionado.Nombre : (formData.terapeuta || '-')}</span>
                        </p>
                        <p className="text-sm font-bold">
                        Fecha: <span className="font-normal">{formData.fecha || '-'}</span>
                        </p>
                        <p className="text-sm font-bold">
                        Hora: <span className="font-normal">{formData.hora || '-'}</span>
                        </p>
                        <p className="text-sm font-bold">
                        Precio Estimado: <span className="font-normal">
                            {servicioSeleccionado ? `$${Number(servicioSeleccionado.Precio).toFixed(2)}` : '-'}
                        </span>
                        </p>
                    </div>
